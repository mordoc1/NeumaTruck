...
function show_card($of, $marcas,$img, $apli, $stk, $ids, $std, $nombre, $cod, $voferta, $vpublicado, $vlista){
    ...
    if ($of != 0) {
      if ( $st_of1 == 1 OR $st_of2 == 1) {
        $html .= '<p style="color:red;margin-top:0px;margin-bottom:0px">Precio Lista <del>'.MonedaTruckIVA($vlista).'</del></p>';
      }
    }else{
      $html .= '<p style="color:red;margin-top:0px;margin-bottom:0px">Precio Lista <del>'.MonedaTruckIVA($vlista).'</del></p>';
    }
