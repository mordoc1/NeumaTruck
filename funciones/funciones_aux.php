...
function getTitle(){
  ...
  $datos	= $mysqli->query("SELECT tipo FROM configuracion WHERE id = 13 ");
...
function diferentOferts(){

  // esta ofertas son las ecepcionales
...
function busqueda_nav_marcas_categoria($categoria){
    ...
    // $re = mysql_query("SELECT DISTINCT m.marca
		...
		// 				INNER JOIN marcas AS m
		// 				ON p.marca = m.id_marcas
		// 				WHERE p.categoria LIKE '$categoria' AND p.v_publicado != 0 AND p.estado = 1 ORDER BY m.prioridad DESC") or die(mysql_error());

    $re = mysql_query("SELECT DISTINCT m.marca
            FROM productos AS p
            INNER JOIN marcas AS m
            ON p.marca = m.id_marcas
            WHERE p.categoria LIKE '$categoria' AND p.estado = 1 AND p.stock >= '$Limite_stock' ORDER BY m.prioridad DESC") or die(mysql_error());
...
function busqueda_nav_medidas_categoria($categoria){
  ...
  $medidas = array();
	...
	// $re = mysql_query("SELECT DISTINCT medidas FROM productos WHERE categoria LIKE '$categoria' AND v_publicado !=0 AND estado = 1 ") or die(mysql_error());
	$re = mysql_query("SELECT DISTINCT medidas FROM productos WHERE categoria LIKE '$categoria' AND estado = 1 AND stock >= '$Limite_stock'") or die(mysql_error());
...
function busqueda_nav_marcas($busqueda){
    ...
    $re = mysql_query("SELECT DISTINCT m.marca
						...
						WHERE p.busqueda LIKE '$busqueda' AND p.v_publicado != 0 AND p.estado = 1 AND p.stock > '$Limite_stock' ORDER BY m.prioridad DESC") or die(mysql_error());
...
function busqueda_nav_medida($busqueda){
	...
	$re = mysql_query("SELECT DISTINCT medidas FROM productos WHERE busqueda LIKE '$busqueda' AND v_publicado !=0 AND estado = 1  AND stock > '$Limite_stock' ") or die(mysql_error());
...
function busquda_producto_categoria($categoria){
  ...
  // $re = mysql_query("SELECT p.id, p.estado, p.codigo , p.nombre, p.stock , m.marca, p.medidas, a.aplicacion, p.media, p.oferta, p.val_oferta, p.v_lista, p.v_publicado
	...
	// 					INNER JOIN marcas AS m
	// 					ON p.marca = m.id_marcas
	// 					WHERE p.categoria LIKE '$categoria' AND p.v_publicado != 0 ORDER BY p.priority ASC ") or die(mysql_error());
  ...
  $re = mysql_query("SELECT p.id, p.estado, p.codigo , p.nombre, p.stock , m.marca, p.medidas, a.aplicacion, p.media, p.oferta, p.val_oferta, p.v_lista, p.v_publicado
						...
						INNER JOIN marcas AS m
						ON p.marca = m.id_marcas
						WHERE p.categoria LIKE '$categoria' AND p.estado = 1 AND p.stock > '$Limite_stock' ORDER BY p.priority ASC ") or die(mysql_error());
...
function busqueda_productos($busqueda){
    ...
    $re = mysql_query("SELECT p.id, p.estado, p.codigo, p.nombre, p.stock , m.marca, p.medidas, a.aplicacion, p.media, p.oferta, p.val_oferta, p.v_lista, p.v_publicado
                      ...
                      INNER JOIN aplicaciones AS a
                      ON p.aplicacion = a.id_nex
                      WHERE p.busqueda LIKE '$busqueda' AND p.v_publicado != 0  AND p.stock > '$Limite_stock' AND p.estado = 1  ORDER BY p.priority ASC ") or die(mysql_error());
      while($f = mysql_fetch_array($re)){
      array_push($productos,array("id" => $f["id"], "estado" => $f["estado"],"codigo" => $f["codigo"], "nombre" => $f["nombre"], "stock" => $f["stock"], "marca" => $f["marca"],
                                  "medidas" => $f["medidas"], "aplicacion" => $f["aplicacion"], "media" => $f["media"], "of" => $f["oferta"], "v_oferta" => $f["val_oferta"],
                                  "v_lista" => $f["v_lista"], "v_publicado" => $f["v_publicado"]));
...
function buscar_marca($busqueda){
    ...
    $re = mysql_query("SELECT p.id, p.estado, p.codigo, p.nombre, p.stock , m.marca, p.medidas, a.aplicacion, p.media, p.oferta, p.val_oferta, p.v_lista, p.v_publicado
                      ...
                      WHERE m.id_marcas = '$busqueda' AND p.v_publicado != 0 AND p.estado = 1 AND p.stock > '$Limite_stock' ") or die(mysql_error());
